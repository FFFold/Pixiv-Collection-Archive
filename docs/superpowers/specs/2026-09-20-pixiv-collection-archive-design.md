...
2. 遇到「整页作品都已在元数据库中存在」停止翻页（稳态下新收藏必在最前几页，通常只请求 1–2 页）。
3. 新出现的作品按 4.2 插入 rank；已存在作品的 rank 不动。
4. 增量模式**不**判定取消收藏（避免误判）。
5. 终止条件只有两个：遇到「整页已知」或列表耗尽。**不设人为页数上限**——一次性新增 300+ 收藏的极端情况会被完整同步，代价仅是本次多请求若干页。
6. 每次同步记录实际请求页数，UI 可见；请求页数异常大（如 > 20 页）时提示考虑执行全量同步。
