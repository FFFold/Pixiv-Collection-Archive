...
- `db/query.py` = shared illust filter core (`IllustFilters` + `build_illust_query` / `build_filtered_pids` / `sort_order`) used by gallery, downloads and export; `web/gallery_query.py` only adds ordering + the display-index window.
